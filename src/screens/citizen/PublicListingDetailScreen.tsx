...
import {formatDateTime, formatCurrency, formatMinimumFraction2} from '../../utils/formatters';
...
const PublicListingDetailScreen = () => {
  ...
  useLayoutEffect(() => {
    const balanceText = walletBalance != null
      ? `JOD ${formatMinimumFraction2(walletBalance.availableBalance)}`
      : '—';
  ...
  const onBuy = () => {
    ...
    if (!walletBalance || walletBalance.availableBalance < property.price) {
      const needed = formatMinimumFraction2(property.price);
      const have   = walletBalance ? formatMinimumFraction2(walletBalance.availableBalance) : '0.00';
      Alert.alert(
    ...
    Alert.alert(
      ...
      `Buy ${property.title} for JOD ${formatMinimumFraction2(property.price)}?\n\nThis will be deducted from your Ejod Wallet.`,
      [
