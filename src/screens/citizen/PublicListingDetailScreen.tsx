...
import {formatDateTime, formatCurrency, formatCurrencyJodMin} from '../../utils/formatters';
...
const PublicListingDetailScreen = () => {
  ...
  const onBuy = () => {
    ...
    if (!walletBalance || walletBalance.availableBalance < property.price) {
      const needed = formatCurrencyJodMin(property.price);
      const have   = walletBalance ? formatCurrencyJodMin(walletBalance.availableBalance) : '0.00';
      Alert.alert(
    ...
    Alert.alert(
      ...
      `Buy ${property.title} for JOD ${formatCurrencyJodMin(property.price)}?\n\nThis will be deducted from your Ejod Wallet.`,
      [
