          ...
          <View>
            ...
            <View style={st.balCard}>
              ...
              <Text style={st.balAmount}>
                ...
                {formatDecimal(balance?.ejodBalance ?? 0)}
              </Text>
              ...
              <View style={st.balTilesRow}>
                <BalanceTile
                  label={w.available}
                  value={`JOD ${formatDecimal(balance?.availableBalance ?? 0)}`}
                  accent
                />
                <View style={st.balTileSep} />
                <BalanceTile
                  label={w.locked}
                  value={`JOD ${formatDecimal(balance?.lockedAmount ?? 0)}`}
                />
                <View style={st.balTileSep} />
                <BalanceTile
                  label={w.pending}
                  value={`JOD ${formatDecimal(balance?.pendingDeposits ?? 0)}`}
                />
              </View>
          ...
          <View style={st.txRow}>
            ...
            <View style={st.txRight}>
              <Text style={[st.txAmount, {color: TX_COLOR[item.type]}]}>
                {TX_SIGN[item.type]}JOD {formatDecimal(item.amount)}
              </Text>
              <View style={[
                st.txStatusBadge,
