...
import {formatDecimalStrict} from '../../utils/formatters';
          ...
          <View>
            ...
            <View style={st.balCard}>
              ...
              <Text style={st.balAmount}>
                ...
                {formatDecimalStrict(balance?.ejodBalance ?? 0)}
              </Text>
              ...
              <View style={st.balTilesRow}>
                <BalanceTile
                  label={w.available}
                  value={`JOD ${formatDecimalStrict(balance?.availableBalance ?? 0)}`}
                  accent
                />
                <View style={st.balTileSep} />
                <BalanceTile
                  label={w.locked}
                  value={`JOD ${formatDecimalStrict(balance?.lockedAmount ?? 0)}`}
                />
                <View style={st.balTileSep} />
                <BalanceTile
                  label={w.pending}
                  value={`JOD ${formatDecimalStrict(balance?.pendingDeposits ?? 0)}`}
                />
              </View>
          ...
          <View style={st.txRow}>
            ...
            <View style={st.txRight}>
              <Text style={[st.txAmount, {color: TX_COLOR[item.type]}]}>
                {TX_SIGN[item.type]}JOD {formatDecimalStrict(item.amount)}
              </Text>
              <View style={[
                st.txStatusBadge,
