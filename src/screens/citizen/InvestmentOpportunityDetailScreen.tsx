...
const InvestmentOpportunityDetailScreen = () => {
  ...
  const onInvest = () => {
    ...
    if (!walletBalance || walletBalance.availableBalance < estimatedTotal) {
      const needed = formatNumber(estimatedTotal, 2);
      const have   = walletBalance
        ? formatNumber(walletBalance.availableBalance, 2)
        : '0.00';
      Alert.alert(
    ...
    const totalFmt = formatNumber(estimatedTotal, 2);
    Alert.alert(
          ...
          right={
            <View style={s.headerBalance}>
              ...
              <Text style={s.headerBalanceText}>
                {walletBalance != null
                  ? `JOD ${formatNumber(walletBalance.availableBalance, 2)}`
                  : '—'}
              </Text>
            </View>
          }
