...
import {formatCurrency, formatMinimumFraction2} from '../../utils/formatters';
...
const InvestmentOpportunityDetailScreen = () => {
  ...
  const onInvest = () => {
    ...
    if (!walletBalance || walletBalance.availableBalance < estimatedTotal) {
      const needed = formatMinimumFraction2(estimatedTotal);
      const have   = walletBalance
        ? formatMinimumFraction2(walletBalance.availableBalance)
        : '0.00';
      Alert.alert(
    ...
    const totalFmt = formatMinimumFraction2(estimatedTotal);
    Alert.alert(
