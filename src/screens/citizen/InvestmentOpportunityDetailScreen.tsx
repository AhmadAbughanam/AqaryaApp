...
import {formatCurrencyNoFraction, formatDecimalMinTwoFraction} from '../../utils/formatters';
...
const InvestmentOpportunityDetailScreen = () => {
  ...
  const onInvest = () => {
    ...
    if (!walletBalance || walletBalance.availableBalance < estimatedTotal) {
      const needed = formatDecimalMinTwoFraction(estimatedTotal);
      const have   = walletBalance
        ? formatDecimalMinTwoFraction(walletBalance.availableBalance)
        : '0.00';
      Alert.alert(
    ...
    const totalFmt = formatDecimalMinTwoFraction(estimatedTotal);
    Alert.alert(
          ...
          right={
            <View style={s.headerBalance}>
              ...
              <Text style={s.headerBalanceText}>
                {walletBalance != null
                  ? `JOD ${formatDecimalMinTwoFraction(walletBalance.availableBalance)}`
                  : '—'}
              </Text>
            </View>
          }
