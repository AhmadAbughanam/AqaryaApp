...
import { formatCurrency, formatMinTwoFractionDigits } from '../../utils/formatters';
...
const InvestmentOpportunityDetailScreen = () => {
  ...
  const onInvest = () => {
    ...
    if (!walletBalance || walletBalance.availableBalance < estimatedTotal) {
      const needed = formatMinTwoFractionDigits(estimatedTotal);
      const have   = walletBalance
        ? formatMinTwoFractionDigits(walletBalance.availableBalance)
        : '0.00';
      Alert.alert(
    ...
    const totalFmt = formatMinTwoFractionDigits(estimatedTotal);
    Alert.alert(
          ...
          right={
            <View style={s.headerBalance}>
              ...
              <Text style={s.headerBalanceText}>
                {walletBalance != null
                  ? `JOD ${formatMinTwoFractionDigits(walletBalance.availableBalance)}`
                  : '—'}
              </Text>
            </View>
          }
