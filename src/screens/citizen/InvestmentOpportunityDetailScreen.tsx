...
import { formatCurrency, formatDecimalMin2 } from '../../utils/formatters';
...
const InvestmentOpportunityDetailScreen = () => {
  ...
  const onInvest = () => {
    ...
    if (!walletBalance || walletBalance.availableBalance < estimatedTotal) {
      const needed = formatDecimalMin2(estimatedTotal);
      const have   = walletBalance
        ? formatDecimalMin2(walletBalance.availableBalance)
        : '0.00';
      Alert.alert(
    ...
    const totalFmt = formatDecimalMin2(estimatedTotal);
    Alert.alert(
          ...
          right={
            <View style={s.headerBalance}>
              ...
              <Text style={s.headerBalanceText}>
                {walletBalance != null
                  ? `JOD ${formatDecimalMin2(walletBalance.availableBalance)}`
                  : '—'}
              </Text>
            </View>
          }
