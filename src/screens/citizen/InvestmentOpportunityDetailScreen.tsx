...
import { formatCurrency, formatFractionMin2 } from '../../utils/formatters';
...
const InvestmentOpportunityDetailScreen = () => {
  ...
  const onInvest = () => {
    ...
    if (!walletBalance || walletBalance.availableBalance < estimatedTotal) {
      const needed = formatFractionMin2(estimatedTotal);
      const have   = walletBalance
        ? formatFractionMin2(walletBalance.availableBalance)
        : '0.00';
      Alert.alert(
    ...
    const totalFmt = formatFractionMin2(estimatedTotal);
    Alert.alert(
