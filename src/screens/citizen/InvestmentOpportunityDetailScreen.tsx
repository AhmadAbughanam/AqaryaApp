...
import {formatCurrency, formatTwoDecimalsMin} from '../../utils/formatters';
...
const InvestmentOpportunityDetailScreen = () => {
  ...
  const onInvest = () => {
    ...
    if (!walletBalance || walletBalance.availableBalance < estimatedTotal) {
      const needed = formatTwoDecimalsMin(estimatedTotal);
      const have   = walletBalance
        ? formatTwoDecimalsMin(walletBalance.availableBalance)
        : '0.00';
      Alert.alert(
    ...
    const totalFmt = formatTwoDecimalsMin(estimatedTotal);
    Alert.alert(
          ...
          right={
            <View style={s.headerBalance}>
              ...
              <Text style={s.headerBalanceText}>
                {walletBalance != null
                  ? `JOD ${formatTwoDecimalsMin(walletBalance.availableBalance)}`
                  : '—'}
              </Text>
            </View>
          }
