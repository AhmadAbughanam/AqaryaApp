...
export const createLocalLLMClient = (
  ...
  const sendPrompt = async (
    ...
    const optionSummary = [
      options.temperature !== undefined ? `temp=${options.temperature}` : null,
      options.maxTokens !== undefined ? `maxTokens=${options.maxTokens}` : null,
    ]
      .filter(Boolean)
      .join(', ');
    ...
    return Promise.resolve(
      `[${config.modelName}] Placeholder response${optionSummary ? ` (${optionSummary})` : ''} for: ${cleanedPrompt}`,
    );
  };
